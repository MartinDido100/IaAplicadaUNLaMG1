import type { RecommendationOutput, RecommendationPromptDto } from "../models/index.js";
...
export class GeminiRecommenderServiceImpl implements MovieRecommendationService {
  constructor() {} 

  async recommendMovies(promptParameters: RecommendationPromptDto): Promise<RecommendationOutput> {
    if (!Constants.GEMINI_API_KEY) throw new Error("Falta GEMINI_API_KEY en variables de entorno.");
    ...
    return this.parseGeminiResponse(response.data?.candidates?.[0]?.content?.parts?.[0]?.text ?? null);
  }
